...
func _draw() -> void:
	...
	var center_roof: Vector2 = Vector2(config.design_size.x * 0.5, 400.0)
	var right_top: Vector2 = Vector2(config.arena_right, config.arena_top)
	draw_line(left_top, center_roof, FRAME_COLOR, 10.0, true)
	draw_line(center_roof, right_top, FRAME_COLOR, 10.0, true)
	...
	draw_dashed_line(Vector2(96.0, 420.0), Vector2(984.0, 420.0), DIVIDER_COLOR, 5.0, 16.0)
	draw_arc(Vector2(config.design_size.x * 0.5, config.arena_bottom + 220.0), 700.0, PI, TAU, 48, FRAME_COLOR, 8.0)
