# covers: [T3-CH-AUTO-01, T3-CH-AUTO-02, T3-CH-AUTO-03, T3-CH-AUTO-04, T3-CH-AUTO-05]
extends SceneTree

const MAIN_SCENE: PackedScene = preload("res://scenes/main.tscn")

func _initialize() -> void:
	await _verify_challenge_launcher_queue_layout()
	await _verify_challenge_wave_flow()
	print("T3 challenge mode test passed: menu gate, launcher queue layout, timed waves, active-ball concurrency, batch separation, and danger stop verified.")
	quit(0)

func _verify_challenge_launcher_queue_layout() -> void:
	...
	controller.config.initial_ball_count = 4
	...
	assert(preview_before_wave.size() == 4, "Challenge mode must begin with the configured initial ball count.")
	var launcher: Launcher = main.get_node("Launcher") as Launcher
	var entry_positions: Array[Vector2] = launcher.get_challenge_preview_positions()
	await physics_frame
	var tossed_positions: Array[Vector2] = launcher.get_challenge_preview_positions()
	var upward_toss_seen: bool = false
	for preview_index: int in mini(entry_positions.size(), tossed_positions.size()):
		upward_toss_seen = upward_toss_seen or tossed_positions[preview_index].y < entry_positions[preview_index].y
	assert(upward_toss_seen, "Challenge previews must use the same upward toss and gravity funnel physics as classic mode.")
	var settling_frames: int = 0
	while not launcher.has_classic_launch_slot_ball() and settling_frames < 180:
		await physics_frame
		settling_frames += 1
	assert(launcher.has_classic_launch_slot_ball(), "One challenge preview ball must naturally settle into the open launch slot.")
	var challenge_preview_positions: Array[Vector2] = launcher.get_challenge_preview_positions()
	var launch_slot_count: int = 0
	for preview_position: Vector2 in challenge_preview_positions:
		if preview_position.is_zero_approx():
			launch_slot_count += 1
		else:
			assert(preview_position.y < -controller.config.ball_radius * 0.5, "Challenge preview balls outside the slot must remain supported above the open launch slot.")
	assert(launch_slot_count == 1, "Only one challenge preview ball may occupy the open launch slot.")
	controller.request_launch(Vector2.DOWN)
	assert(manager.get_active_ball_count() == 1, "Challenge mode must launch only the ball occupying the open slot first.")
	var previous_preview_count: int = launcher.get_waiting_ball_count()
	assert(previous_preview_count == 3, "The challenge launcher preview must remove the first ball immediately after it launches.")
	var preview_queue_reduced: bool = false
	var launch_frames: int = 0
	while manager.get_active_ball_count() < 4 and launch_frames < 360:
		await physics_frame
		var current_preview_count: int = launcher.get_waiting_ball_count()
		preview_queue_reduced = preview_queue_reduced or current_preview_count < previous_preview_count
		previous_preview_count = current_preview_count
		launch_frames += 1
	assert(manager.get_active_ball_count() == 4, "Remaining challenge balls must naturally refill the slot and then launch in sequence.")
	assert(preview_queue_reduced and launcher.get_waiting_ball_count() == 0, "Challenge preview balls must be removed after every sequential launch instead of retaining a stale queue.")
	main.queue_free()
	await process_frame

func _verify_challenge_wave_flow() -> void:
	var main: Main = MAIN_SCENE.instantiate() as Main
	root.add_child(main)
	await process_frame
	var controller: GameController = main.get_node("GameController") as GameController
	...
	var launcher: Launcher = main.get_node("Launcher") as Launcher
	assert(launcher.get_presentation() == Launcher.Presentation.CHALLENGE, "Challenge mode must never switch to the classic closed-launcher presentation.")
